...
uint32_t rgb_to_rgba(uint32_t x) { return x | 0xFF; }
...
int decode(const std::vector<uint8_t> &input, Image &output) {
  ...
  uint32_t row_size = (((width * nbits) + 31) & ~31) /
                      8; // Row size in bits, divided by input datatype width

  const auto *palette =
      reinterpret_cast<const uint32_t *>(&input[14 + dib_size]);
  const auto *ptr = &input[offset];

  uint32_t abs_heigh = std::abs(height);
  for (uint32_t y = 0; y < abs_heigh; ++y) {
    int yy = (0 < height) ? (abs_heigh - y - 1) : y;
