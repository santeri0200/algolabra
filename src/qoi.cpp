...
// ===
//
// INDEX includes 2-bit tag and 6-bit index into the ColorData array for already
// seen colors.
//
// +-INDEX-------------------------+
// | 7   6   5   4   3   2   1   0 |
...
// ===
//
// LUMA includes 2-bit tag, 6-bit wrapping difference for green and 4-bit
// wrapping differences for red and blue from green's difference from the last
// color. Green channel has a bias of 32, and red and blue have a bias of 8.
//
// +-LUMA--------------------------+-------------------------------+
...
__inline__ int chech_header_validity(Headers headers) {
  ...
  bool equal = std::equal(std::begin(magic), std::end(magic),
                          std::begin(headers.structure.magic));
...
int decode(const std::vector<uint8_t> &input, Image &output) {
  ...
  if (chech_header_validity(headers) != 0) {
    return -1;
  }
  ...
  while (i < input.size()) {
    ...
    case 0x80 ... 0xBF:
      current_color.r =
          (uint8_t)(current_color.r + (input.at(i) & 0b00111111) - 32);
      current_color.g =
          (uint8_t)(current_color.g + (input.at(i) & 0b00111111) - 32);
      current_color.b =
          (uint8_t)(current_color.b + (input.at(i) & 0b00111111) - 32);
      i++;
...
int encode(const Image &img, std::vector<uint8_t> &output) {
  ...
  for (uint32_t i = 0; i < size; i++) {
    ...
    if (pixel == previous_color) {
      run++;
      // QOI run encode is setup as `11xxxxxx`. If run is equal to 63 or 64, the
      // byte overlaps with RGB or RGBA commands. Therefore it is limited to 62.
      if (run == 62 || i == size - 1) {
