...
void pushChunk(std::vector<uint8_t> &png, const char type[4],
...
int decode(const std::vector<uint8_t> &input, Image &output) {
  ...
  while (pos + 12 <= input.size()) {
    ...
    if (type == 0x49484452) {
      ...
      idat.insert(idat.end(), input.begin() + pos,
  ...
  for (uint32_t y = 0; y < height; ++y) {
    ...
    for (uint32_t x = 0; x < stride; ++x) {
      ...
      uint8_t upLeft =
          (channels <= x && 0 < y) ? raw[((y - 1) * stride) + x - channels] : 0;
...
int encode(const Image &img, std::vector<uint8_t> &output) {
  ...
  DeflateStored(filtered.data(), filtered.size(), compressed);
